...
pki: root intermediate server-external-authz client-john@test.com
...
server-%:
	openssl req -newkey rsa:2048 -keyout "intermediate/private/server-$*.key" -out "intermediate/csr/server-$*.csr" -nodes -subj "/C=AU/ST=NSW/L=Sydney/O=Test/CN=$*"
	openssl x509 -req -CA "intermediate/certs/ca.crt" -CAkey "intermediate/private/ca.key" -in "intermediate/csr/server-$*.csr" -out "intermediate/certs/server-$*.crt" -set_serial $$(openssl rand 10 | od -DAn | tr -d ' ' | tr -d '\n') -extfile intermediate/openssl.conf -extensions server_cert -days 180

client-%:
	openssl req -newkey rsa:2048 -keyout "intermediate/private/client-$*.key" -out "intermediate/csr/client-$*.csr" -nodes -subj "/C=AU/ST=NSW/L=Sydney/O=Test/CN=$*"
	openssl x509 -req -CA "intermediate/certs/ca.crt" -CAkey "intermediate/private/ca.key" -in "intermediate/csr/client-$*.csr" -out "intermediate/certs/client-$*.crt" -set_serial $$(openssl rand 10 | od -DAn | tr -d ' ' | tr -d '\n') -extfile intermediate/openssl.conf -extensions client_cert -days 180
